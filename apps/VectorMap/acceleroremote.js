		...
		return [
			speakergrid({flex:1, padding: 70},noisegrid({padding:20,borderradius: 20, justifycontent: "center"}, 
				...
				view({bg:0, alignself:"center", flexdirection:"column"}
					...
					,button({icon:"arrows",margin: 10, padding:20, mouseleftdown : function(){this.setAccelTarget("Move");}.bind(this), mouseleftup: function(){this.setAccelTarget("");}.bind(this)})
					,button({icon:"video-camera",margin: 10, padding:20, mouseleftdown : function(){this.setAccelTarget("Pan");}.bind(this), mouseleftup: function(){this.setAccelTarget("");}.bind(this)})
				)
			)
			)
		]
